...
# Save the distribution plot
ggsave("Data/IQ_Plot/7_Heatmap_top_plot.png", plot = Distribution_top_plot, width = 15, height = 8)
...
ggsave("Data/IQ_Plot/9_Proportion_Bubble_plot.png", plot = PPlot, width = 15, height = 8)
...
ggsave("Data/IQ_Plot/10_IQ_BiNI_plot.png", plot = PPlot, width = 15, height = 8)
...
MQS_score_NPAtlas <- MQS_score_NPAtlas %>% select(id, MQScore, ATTRIBUTE_strain)
...
for (sample_name in unique(MQS_score_NPAtlas$ATTRIBUTE_strain)) {
  individual_dfs_NPAtlas <- score_MQS %>%
    filter(ATTRIBUTE_strain == sample_name) %>%
    select(id, MQScore)
  
  # Count unique values in each stratum for MQScore
  MQScore_counts_NPAtlas <- stratify_and_count_unique(individual_dfs_NPAtlas, "MQScore", strata_MQS)
  ...
  # Assign the individual dataframe to the list
  individual_dfs_NPAtlas[[sample_name]] <- individual_dfs_NPAtlas
}

## Save each individual dataframe and the strata counts to CSV files in the CSV_IQ folder
for(sample_name in names(individual_dfs_NPAtlas)) {
  write.csv(individual_dfs_NPAtlas[[sample_name]], file = paste0("Data/CSV_IQNPA/", sample_name, "_data_MQS_NPAtlas.csv"), row.names = FALSE)
  write.csv(strata_counts_NPAtlas[[sample_name]], file = paste0("Data/CSV_IQNPA/", sample_name, "_strata_counts_MQS_NPAtlas.csv"), row.names = FALSE)
  
  # Assign each dataframe to a variable in the R environment
  assign(paste0(sample_name, "_data"), individual_dfs_NPAtlas)
  assign(paste0(sample_name, "_strata_counts"), counts_df_NPAtlas)
}
